...
import { STEP_COUNT_REGEX_STRING, HOURS_SLEPT_REGEX_STRING, WEIGHT_REGEX_STRING, parseVariable, DATASOURCE_VARIABLE_RULES } from "./preprocessor-templates"
...
export async function inferHighlight(nlp: compromise.Document, original: string, guidedDataSource: DataSourceType | undefined, options: NLUOptions): Promise<{ conditionInfo: ConditionInfo, match: compromise.Document | string } | null> {
    ...
    if (durationComparisonInfo) {
        ...
        if (numericComparisonInfo) {
            ...
            //goal accomplishment
            const goalPartRegex = `((a|our|my|the)\\s+)?(current\\s+)?((?<source>${STEP_COUNT_REGEX_STRING}|${WEIGHT_REGEX_STRING})\\s+)?(goal|gold?|call|girl)`

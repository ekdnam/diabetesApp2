import {
    createGoToBrowseRangeAction,
    memoUIStatus,
    ExplorationAction,
    createGoToBrowseDayAction,
    setDataDrivenQuery,
...
import {
    DataSourceChartFrame,
    HEADER_HEIGHT,
    FOOTER_HEIGHT,
} from '@components/exploration/DataSourceChartFrame';
...
} from '@components/visualization/browse/common';
...
class OverviewMainPanel extends React.PureComponent<Props, State> {
    static getDerivedStateFromProps(
        ...
        if (nextProps.data != null && nextProps.data.range != null) {
            ...
            if (
                currenDomain[0] !== nextProps.data.range[0] ||
                currenDomain[1] !== nextProps.data.range[1]
            ) {
                return {
                    ...currentState,
                    scaleX: CommonBrowsingChartStyles.makeDateScale(
                        currentState.scaleX.copy(),
                        nextProps.data.range[0],
                        nextProps.data.range[1],
                    ),
                };
            } else return null;
        } else return null;
    ...
    constructor(props: Props) {
        super(props);
        console.log('DATA_SOURCE_LIST::::', this.props.data.sourceDataList);
        this.state = {
            scaleX: CommonBrowsingChartStyles.makeDateScale(
                undefined,
                props.data.range[0],
    ...
    componentDidMount() {
        ...
        if (
            this._listRef.current != null &&
            this.props.overviewScrollY != null
        ) {
            requestAnimationFrame(() => {
                this._listRef.current.scrollToOffset({
                    offset: this.props.overviewScrollY,
                    animated: false,
                });
            });
        }
        ...
        if (this.props.data != null) {
            const viewableDataSources = this.props.data.sourceDataList.map(
                (d) => d.source,
            );
            this.props.dispatchAction(
                memoUIStatus('viewableDataSources', viewableDataSources),
            );
        }
    }
    ...
    componentDidUpdate(prevProps: Props) {
        if (prevProps.dataDrivenQuery !== this.props.dataDrivenQuery) {
            ...
            if (
                this.props.dataDrivenQuery != null &&
                (prevProps.dataDrivenQuery == null ||
                    prevProps.dataDrivenQuery.dataSource !==
                        this.props.dataDrivenQuery.dataSource)
            ) {
                this._listRef.current?.scrollToIndex({
                    animated: true,
                    index: this.props.data.sourceDataList.findIndex(
                        (d) =>
                            d.source === this.props.dataDrivenQuery.dataSource,
                    ),
                });
            }
        }
        ...
        if (
            ...
            this.state.refreshingSince != null
        ) {
            if (this.currentTimeoutForRefreshingFlag) {
                clearTimeout(this.currentTimeoutForRefreshingFlag);
            }

            const minLoadingTimeLeft = Math.max(
                MIN_REFRESH_TIME_FOR_PERCEPTION,
                Date.now() - this.state.refreshingSince,
            );
    ...
    componentWillUnmount() {
        ...
        this.props.dispatchAction(
            memoUIStatus('overviewScrollY', this.currentListScrollOffset),
        );
        this.props.dispatchAction(
            memoUIStatus('viewableDataSources', undefined),
        );
    }

    private readonly onDiscardFilter = () => {
        this.props.dispatchAction(
            setDataDrivenQuery(InteractionType.TouchOnly, null),
        );
    };

    private readonly onFilterModified = (newFilter: DataDrivenQuery) => {
        this.props.dispatchAction(
            setDataDrivenQuery(InteractionType.TouchOnly, newFilter),
        );
    };

    private readonly onHeaderPressed = (source: DataSourceType) => {
        this.props.dispatchAction(
            createGoToBrowseRangeAction(InteractionType.TouchOnly, source),
        );
    };
    ...
    private readonly onTodayPressed = (source: DataSourceType) => {
        this.props.dispatchAction(
            createGoToBrowseDayAction(
                ...
                inferIntraDayDataSourceType(source),
                DateTimeHelper.toNumberedDateFromDate(
                    this.props.selectedService.getToday(),
                ),
            ),
        );
    };
    ...
    private readonly getItemLayout = (_: any, index: number) => {
        const height =
            CommonBrowsingChartStyles.CHART_HEIGHT +
            HEADER_HEIGHT +
            FOOTER_HEIGHT +
            separatorStyle.height;
    ...
    private readonly renderItem = ({item}: {item: OverviewSourceRow}) => (
        <DataSourceChartFrame
            ...
            onTodayPressed={
                inferIntraDayDataSourceType(item.source) != null
                    ? this.onTodayPressed
                    : null
            }
        />
    ...
    private readonly onViewableItemsChanged = (args: {
        viewableItems: Array<ViewToken>;
        changed: Array<ViewToken>;
    }) => {
        const viewableDataSources = args.viewableItems.map(
            (token) => token.key as DataSourceType,
        );
        this.props.dispatchAction(
            memoUIStatus('viewableDataSources', viewableDataSources),
        );
    ...
    render() {
        if (this.props.data != null) {
            return (
                <DateRangeScaleContext.Provider value={this.state.scaleX}>
                    {this.props.dataDrivenQuery != null ? (
                        <DataDrivenQueryBar
                            filter={this.props.dataDrivenQuery}
                            highlightedDays={this.props.data.highlightedDays}
...
function mapStateToProps(state: ReduxAppState, ownProps: Props): Props {
    const selectedService = DataServiceManager.instance.getServiceByKey(
        state.settingsState.serviceKey,
    );
...
function mapDispatchToProps(
    ...
    return {
        ...
        dispatchDataReload: () =>
            dispatch(startLoadingForInfo(undefined, true)),
    };
}

const overviewMainPanel = connect(
    mapStateToProps,
    mapDispatchToProps,
)(OverviewMainPanel);

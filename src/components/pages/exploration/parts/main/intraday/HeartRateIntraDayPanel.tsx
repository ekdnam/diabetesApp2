...
const heartRateZoneSpecs: Array<{
    type: HeartRateZone;
    color: string;
    name: string;
}> = [
...
export const HeartRateIntraDayPanel = React.memo(() => {
    ...
    if (data != null && data.points.length > 0) {
        ...
        const scaleY = scaleLinear()
            .domain([
                min(data.points, (d) => d.value),
                max(data.points, (d) => d.value),
            ])
            .range([chartArea.height, 0])
            .nice();

        const line = d3Shape
            .line<IIntraDayHeartRatePoint>()
        ...
        const clusters = dbscan
            .run(
                ...
                900,
                2,
                (a: any, b: any) => b.secondOfDay - a.secondOfDay,
            )
            .map((cluster: Array<number>) =>
                cluster.map((index) => data.points[index]),
            );

        const exerciseZones = data.zones.filter(
            (zone) => zone.name != HeartRateZone.OutOfRange,
        );
        ...
        return (
            <ScrollView style={styles.containerStyle}>
                <SizeWatcher
                    containerStyle={styles.chartContainerStyle}
                    ...
                    <Svg
                        ...
                        <G {...chartArea}>
                            ...
                            <Line
                                x1={0}
                                x2={chartArea.width}
                                y={scaleY(data.restingHeartRate)}
                                stroke={Colors.chartAvgLineColor}
                                strokeWidth={2}
                                strokeDasharray={'4'}
                            />
                            <SvgText
                                y={scaleY(data.restingHeartRate) - 8}
                ...
                <SummaryTableRow
                    ...
                    value={
                        data.restingHeartRate
                            ? [
                                  {type: 'value', value: data.restingHeartRate},
                                  {type: 'unit', value: ' bpm'},
                              ]
                            : [{type: 'unit', value: 'Cannot calculate yet.'}]
                    }
                />
                ...
                <View
                    style={{
                        ...
                        marginTop: 0,
                        height: 1,
                        backgroundColor: Colors.chartLightText,
                    }}></View>
                ...
                {heartRateZoneSpecs.map((spec) => {
                    ...
                    if (zoneInfo != null) {
                        ...
                        return (
                            <View
                                key={spec.type}
                                style={styles.zoneChartContainerStyle}>
                                <Text style={styles.zoneChartTitleStyle}>
                                    {spec.name}
                                </Text>
                                <SizeWatcher
                                    containerStyle={{
                                ...
                                />
                                <Text style={styles.zoneChartValueStyle}>
                                    {zoneInfo
...
const SummaryTableRow = (prop: {
    title: string;
    value: Array<{type: 'unit' | 'value'; value: string | number}>;
}) => {
    return (
        <View style={commonIntraDayPanelStyles.summaryRowContainerStyle}>
            <Text
                style={{
                    ...commonIntraDayPanelStyles.summaryTextTitleStyle,
                    flex: 1,
                }}>
                {prop.title}
            </Text>
            <Text
                ...
                {prop.value.map((entity, i) => (
                    <Text
                        key={i.toString()}
                        style={
                            entity.type === 'unit'
                                ? commonIntraDayPanelStyles.summaryTextUnitStyle
                                : null
                        }>
                        {entity.value}
                    </Text>

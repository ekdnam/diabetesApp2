...
import {
    ExplorationAction,
    createGoToBrowseOverviewAction,
    createRestorePreviousInfoAction,
    goBackAction,
...
const undoIconStyle = (
    <SvgIcon type={SvgIconType.Reset} size={20} color={Colors.WHITE} />
);
...
class ExplorationScreen extends React.PureComponent<ExplorationProps, State> {
    ...
    private speechFeedbackRef = React.createRef<
        SpeechEventNotificationOverlay
    >();
    ...
    private onAppStateChange = async (nextAppState: AppStateStatus) => {
        if (
            ...
            if (
                this.state.prepareStatus === PrepareStatus.ACQUIRING_PERMISSION
            ) {
                await this.prepare();
            }
        } else if (
    ...
    private forwardUserToPermissionSettings(
        title: string,
        content: string,
    ): Promise<void> {
    ...
    private async checkPermission(): Promise<
        'granted' | 'denied' | 'forwarded'
    > {
        //permissions
        if (Platform.OS === 'ios') {
            let [
            ...
            if (
                ...
                speechRecognitionPermissionStatus === RESULTS.GRANTED
            ) {
                console.log('All required permissions are met.');
                return 'granted';
            } else if (
                microphonePermissionStatus === RESULTS.BLOCKED &&
                speechRecognitionPermissionStatus === RESULTS.BLOCKED
            ) {
                console.log('Both permissions are blocked.');
                await this.forwardUserToPermissionSettings(
                    'Permissions required',
                    'Please grant permission for microphone and speech recognition in the settings.',
                );
                return 'forwarded';
            } else {
                if (microphonePermissionStatus === RESULTS.BLOCKED) {
                    ...
                    await this.forwardUserToPermissionSettings(
                        ...
                        'Please grant permission for the microphone access.',
                    );
                    return 'forwarded';
                } else if (
                    microphonePermissionStatus !== RESULTS.GRANTED &&
                    microphonePermissionStatus !== RESULTS.UNAVAILABLE
                ) {
                ...
                if (speechRecognitionPermissionStatus === RESULTS.BLOCKED) {
                    ...
                    await this.forwardUserToPermissionSettings(
                        ...
                        'Please grant permission for speech recognition.',
                    );
                    return 'forwarded';
                } else if (
                    speechRecognitionPermissionStatus !== RESULTS.GRANTED &&
                    speechRecognitionPermissionStatus !== RESULTS.UNAVAILABLE
                ) {
                    speechRecognitionPermissionStatus = await request(
                        PERMISSIONS.IOS.SPEECH_RECOGNITION,
                    );
                }
            ...
            let audioRecordPermissionStatus = await check(
            ...
            if (audioRecordPermissionStatus === RESULTS.GRANTED) {
                ...
                audioRecordPermissionStatus = await request(
                    PERMISSIONS.ANDROID.RECORD_AUDIO,
                );
                return this.checkPermission();
    ...
    async componentDidMount() {
        ...
        this.subscriptions.add(
            SpeechEventQueue.instance.onNewEventPushed.subscribe(
                (event) => {
    ...
    private async prepare() {
        ...
        if (permissionResult === 'granted') {
            ...
            console.log(
                'I will wait the user to return from the permission settings.',
            );
        } else {
    ...
    private async performServiceActivationPhase() {
        ...
        if (Platform.OS === 'ios') {
            console.log(
                'App is currently not in foregroud. Defer the activation to the app state listener.',
            );
            while (AppState.currentState !== 'active') {
                await sleep(100);
            }
            console.log(
                'Okay now the app is in foregroud. Try activation now.',
            );
        }

        try {
            const serviceActivationResult = await DataServiceManager.instance
                .getServiceByKey(this.props.selectedServiceKey)
                .activateInSystem((progressInfo) => {
                    this.setState({
                        ...this.state,
    ...
    async componentDidUpdate(prevProps: ExplorationProps, prevState: State) {
        ...
        const isExplorationInfoChanged =
            explorationInfoHelper.equals(
                prevProps.explorationInfo,
                this.props.explorationInfo,
            ) === false;

        if (
            this.props.explorationInfo.type !==
                prevProps.explorationInfo.type ||
            isExplorationInfoChanged === true
        ) {
    ...
    onBottomBarButtonPress = (mode: ExplorationMode) => {
        switch (mode) {
            case ExplorationMode.Browse:
                this.props.dispatchCommand(
                    createGoToBrowseOverviewAction(InteractionType.TouchOnly),
                );
                break;
    ...
    onGlobalSpeechInputPressOut = () => {
        ...
        this.props.dispatchSetShowGlobalPopup(
            false,
            this.state.globalSpeechSessionId,
        );
    ...
    render() {
        return (
            <TodayContext.Provider
                value={DateTimeHelper.toNumberedDateFromDate(
                    DataServiceManager.instance
                        .getServiceByKey(this.props.selectedServiceKey)
                        .getToday(),
                )}>
                <View style={StyleTemplates.screenDefaultStyle}>
                    ...
                    {this.state.prepareStatus === PrepareStatus.FAILED ? (
                        <View
                            style={[
                                styles.mainContainerStyle,
                                styles.perpareFailedContainerStyle,
                            ]}>
                            <Text style={styles.prepareFailedTextStyle}>
                                Activate to use Data@Hand.
                            </Text>
                            <Button
                                type="outline"
                                title="Activate Again"
                                onPress={this.onPressActivateButton}
                            />
                        </View>
                    ) : (
                        <>
                            <View style={styles.mainContainerStyle}>
                                ...
                                {this.props.isDataLoading === true && (
                                    <BusyHorizontalIndicator />
                                )}

                                {this.props.loadedDataInfo != null ? (
                                    this.makeMainPanel(
                                        this.props.loadedDataInfo.type,
                                    )
                                ) : (
                                    <></>
                                )}
                                ...
                                {/* history panel ===================================================================*/}
                                {this.props.isUndoAvailable === true &&
                                    this.state.undoIgnored === false && (
                                        <View style={styles.historyPanelStyle}>
                                            <Button
                                                ref={this.speechUndoButtonRef}
                                                key="undo"
                                                containerStyle={
                                                    styles.historyButtonContainerStyle
                                                }
                                                buttonStyle={
                                                    styles.historyButtonStyle
                                                }
                                                icon={undoIconStyle}
                                                onPress={this.undo}
                                                title="Undo speech"
                                                titleStyle={
                                                    styles.historyButtonTitleStyle
                                                }
                                            />
                                        </View>
                                    )}
                            ...
                            <BottomBar
                                mode={explorationInfoHelper.getMode(
                                    this.props.explorationInfo,
                                )}
                                onModePress={this.onBottomBarButtonPress}
                                onVoiceButtonPressIn={
                                    this.onGlobalSpeechInputPressIn
                                }
                                onVoiceButtonPressOut={
                                    this.onGlobalSpeechInputPressOut
                                }
                            />

                            <TouchSafeBottomSheet
                                ref={this.comparisonBottomSheetRef}>
                                <ComparisonInitPanel
                                    info={this.props.explorationInfo}
                                    onCompleted={() => {
                                        this.comparisonBottomSheetRef.current?.close();
                                    }}
                                />
                            </TouchSafeBottomSheet>
                            ...
                            <TooltipOverlay />
                            <GlobalSpeechOverlay
                                isGlobalSpeechButtonPressed={
                                    this.props.showGlobalSpeechPopup
                                }
                            />
    ...
    makeMainPanel(type: ExplorationType): any {
        switch (type) {
            ...
            case ExplorationType.C_CyclicDetail_Range:
                return (
                    <MultiRangeComparisonMainPanel
                        noDataMessageOverride={`No data exist. Please check whether the current range includes <b>${
                            getCycleDimensionSpec(
                                explorationInfoHelper.getParameterValue(
                                    this.props.loadedDataInfo,
...
function mapDispatchToProps(
    ...
    ownProps: ExplorationProps,
): ExplorationProps {
    return {
        ...ownProps,
        dispatchCommand: (command: ExplorationAction) => dispatch(command),
        dispatchDataReload: (info: ExplorationInfo) =>
            dispatch(startLoadingForInfo(info)),
        dispatchStartSpeechSession: (sessionId, currentExplorationType) =>
            dispatch(
                startSpeechSession(
                    sessionId,
                    SpeechContextHelper.makeGlobalContext(
                        currentExplorationType,
                    ),
                ),
            ),
        dispatchFinishDictation: (sessionId) =>
            dispatch(requestStopDictation(sessionId)),
        dispatchSetShowGlobalPopup: (value, sessionId) =>
            dispatch(createSetShowGlobalPopupAction(value, sessionId)),
...
const explorationScreen = connect(
    mapStateToProps,
    mapDispatchToProps,
)(ExplorationScreen);
